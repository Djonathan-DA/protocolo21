...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'UserProgress' },
        () => fetchData()
      )
      .subscribe()
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className={`fixed md:relative top-16 md:top-0 w-64 h-full glass-panel border-r border-t-0 border-l-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 z-40 md:pt-16`}>
        <div className="py-6 flex flex-col gap-1 h-full overflow-y-auto">
          {/* Home Link */}
          <Link href="/home" onClick={() => setIsOpen(false)} className={`px-6 py-3 text-sm flex items-center gap-3 transition-colors ${pathname === '/home' ? 'text-gold bg-gold/10 border-r-2 border-gold' : 'text-gray-400 hover:text-white'}`}>
            <Home size={18} /> Início
          </Link>

          {/* Modules Section Header */}
          <div className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-gold/50 mt-4">A Jornada</div>
              ...
              <div key={mod}>
                ...
                <button
                  ...
                  className={`w-full px-6 py-3 text-sm flex items-center justify-between transition-colors ${
                    pathname.includes(`/modules/${daysInModule[0]?.day}`) || pathname.includes(`/modules/${daysInModule[daysInModule.length - 1]?.day}`)
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold">Módulo {mod}: {moduleNames[mod as keyof typeof moduleNames]}</span>
                  </div>
                  <ChevronDown
                    size={16}
          ...
          {/* Tools Section */}
          <div className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-gold/50 mt-4">Ferramentas</div>
          <Link href="/checklist" onClick={() => setIsOpen(false)} className={`px-6 py-3 text-sm flex items-center gap-3 transition-colors ${pathname === '/checklist' ? 'text-gold bg-gold/10 border-r-2 border-gold' : 'text-gray-400 hover:text-white'}`}>
            <CheckSquare size={18} /> Checklist Diário
          </Link>

          {/* VIP Bonus Section */}
          <div className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-gold/50 mt-4">Bônus VIP</div>
          <Link href="/bonus" onClick={() => setIsOpen(false)} className={`px-6 py-3 text-sm flex items-center gap-3 transition-colors ${pathname === '/bonus' ? 'text-gold bg-gold/10 border-r-2 border-gold' : 'text-gray-400 hover:text-white'}`}>
